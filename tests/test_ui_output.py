...
def test_menu_folder_typo_then_retry_shows_count(
    ...
    # → Enter skips the post-scan redaction offer → Enter → 9 quit.
...
def test_menu_redact_requires_typed_confirmation(
    ...
    _feed_menu(monkeypatch, ["5", "redact", "", "9"])
    assert main([]) == 0
...
def test_menu_redact_confirmed_writes_and_undo_restores(
    ...
    # Enter → 6 undo → y → Enter → 9 quit.
    _feed_menu(monkeypatch, ["5", "REDACT", "y", "", "6", "y", "", "9"])
    assert main([]) == 0
