...
def menu_options() -> None:
    ...
    grid.add_row("[4]", "Scan Cursor history", "read-only")
    grid.add_row("[5]", "Scan Windsurf history", "read-only")
    grid.add_row("[6]", "Scan Aider history", "read-only")
    grid.add_row("[7]", "Scan Cline history", "read-only")
    grid.add_row("[8]", "Scan Gemini CLI history", "read-only")
    grid.add_row("[9]", "Scan Continue history", "read-only")
    grid.add_row("[10]", "Scan GitHub Copilot Chat history", "read-only")
    grid.add_row("[11]", "Scan a custom folder", "read-only")
    grid.add_row("[12]", "Redact Claude Code history", "asks to confirm · .bak backups")
    grid.add_row("[13]", "Undo last redaction", "restores .bak backups")
    grid.add_row("[14]", "Findings as JSON", "read-only")
    grid.add_row("[15]", "Check for updates", "")
    grid.add_row("[16]", "Quit", "")
